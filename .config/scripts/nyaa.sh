...
if [ -z "$q" ] ; then
	...
	url='?f=2&c=1_2&q=/'"$q"'&s=seeders&o=desc'
fi

# get results
# Here be dragons!
r=`curl -k -A Mozilla -b "lw=s" -m 15 -s "$NYAA/$url" \
			| grep -Eo '<a href=[^>]+|<td class=[^<]+' \
			| sed -n '/<td/p; /magnet/p; /title="/p' \
			| sed 's/^.*center">//; s/^.*title="//; s/^.*<a href="//; s/".*$//' \
			| sed '/English-translated/d; /<td class=/d; /comment/d;' \
			| sed  'N;N;N;N;N;N;s!\n!|!g'`
# number of results
n=`echo "$r" | wc -l`
...
echo "$r" \
	...
	| awk -v N=1 \
		...
		-v NO="$nonecolor" \
		-F '|' \
		'{print NU N ") " NA $1 " " SI $5 " " SE $6 " " PE $4 NO; N++}'
...
echo -n "\033[44;1;37mDownloading torrent:\033[0m"
for torrent in $down ; do
	...
	if [ $torrent -ge 1 ] ; then
		if [ $torrent -le $limit ] ; then
			name=`echo "$r" | awk -F '|' 'NR=='$torrent'{print $1; exit}'`
			echo -n "\033[41;1;37m $torrent = $name\033[0m"
			command="$program `echo "$r" | awk -F '|' 'NR=='$torrent'{print $3; exit}'`"
