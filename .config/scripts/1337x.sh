...
echo "$r" \
	...
	| awk -v N=1 \
		...
		-v NO="$nonecolor" \
		-F '|' \
		'{print NU N ") " NA $2 " " SE $3 " " PE $4 " " SI $5 NO; N++}'
...
for torrent in $down ; do
 	...
 	if [ $torrent -ge 1 ] ; then
 		if [ $torrent -le $limit ] ; then
			name=`echo "$r" | awk -F '|' 'NR=='$torrent'{print $2}'`
 			echo -n "\033[41;1;37m $torrent = $name\033[0m"
			magnet=`echo "$r" | awk -F '|' 'NR=='$torrent'{print $1}'`
 			m=`curl -k -A Mozilla -b "lw=s" -m 15 -s "$TPB""$magnet"\
 			| grep -Eoi '<li><[^*]+</li>' \
 			| sed 's!^href=magnet[^>]*>!!; \
 			s/^.*href="//; \
 			s/" onclick.*$//; \
